...
to a full press release. It must appear from **today (17 July 2026) through
31 July 2026** and disappear on its own — no human cleanup.

The AGM is Friday 31 July 2026, 08:00, Mabeyo Social Hall, Dodoma.
...
| Dismissal | Forever, until window ends | `localStorage`, keyed to the announcement identity so a future card gets a clean slate. |
| Date evaluation | Server-side, EAT (UTC+3) | Avoids browser-timezone drift showing the card early/late. |
...
announcementCard (group)
  ...
  startDate  date  (dayAndTime)  ← 2026-07-17
  endDate    date  (dayAndTime)  ← 2026-07-31 (inclusive through EOD EAT)
...
- `image` is an `upload` relationship to the `media` collection (resolved at
  `depth: 1`).
- `endDate` is treated as inclusive through 23:59:59 EAT of that calendar day.
...
- Evaluates the window server-side in EAT: compute "now in UTC+3", render only
  when `active && startDate ≤ now ≤ endDate` (end inclusive to 23:59:59 EAT).
- Outside the window / inactive / no image / no title → `null`.
...
- `dismissKey` = a short hash of `title + endDate`, so re-using the group for a
...
- Window math is date-only-safe: an admin who sets only `active` with no dates
  is treated as **not in window** (both dates required to show), preventing an
  accidental permanent card.

## Testing / Verification
1. Set `active` + dates spanning today → card slides in bottom-right after ~1.2s.
...
4. Set `endDate` to yesterday → card does not render (server returns null,
   confirm zero announcement JS in the network tab).
5. Set `startDate` to tomorrow → card does not render.
