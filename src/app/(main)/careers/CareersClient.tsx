...
export default function CareersClient({ jobs }: { jobs: JobListing[] }) {
  ...
  const filteredJobs =
    activeFilter === "All"
      ? jobs
      : jobs.filter((j) => j.job_type === activeFilter);

  const toggleJob = (idx: number) =>
    setExpandedJobIdx((prev) => (prev === idx ? null : idx));

const DEPT_ICONS: Record<string, React.ElementType> = {
  Technology: Code, Operations: Briefcase, Credit: BarChart3,
  Marketing: Megaphone, "ICT Security": Shield, "Business Banking": Handshake,
  "IT & Digital Transformation": Cpu, HR: Users, Finance: DollarSign,
};
const getDeptIcon = (dept: string): React.ElementType => DEPT_ICONS[dept] || Briefcase;
                            ...
                            <div className="mt-6 rounded-xl border border-gray-200 bg-white p-6">
                              ...
                              <p className="text-sm leading-relaxed text-body">
                                ...
                                <span className="font-semibold text-navy">
                                  &ldquo;Application &ndash; {job.job_title}&rdquo;
                                </span>
                                .
                              </p>
