...
function getBranchStatus(
  ...
  if (!hours) return { label: "Closed", open: false };
...
const branches: Branch[] = [
  {
    ...
    city: "Dodoma",
    region: "Dodoma",
    address: "Kuu Street, P.O. Box 201, Dodoma",
    hours: { weekday: "Mon\u2013Fri: 8:30AM\u20134:00PM", saturday: "Sat: 8:30AM\u20131:30PM" },
...
const comingSoonBranches = branches.filter((b) => b.comingSoon);
          ...
          {(() => {
            ...
            return (
              <>
                ...
                >
                  {filteredActive.map((branch, i) => {
                    const isOpen = expandedId === branch.id;
                    const status = getBranchStatus(branch.hours);
                    const mapsQuery = encodeURIComponent(
                      branch.address
                        ? `${branch.name}, ${branch.address}`
                        : `${branch.name}, ${branch.city}, Tanzania`
                    );
                        ...
                        >
                          <div className="flex items-center gap-4">
                            ...
                            >
                              <Building2
                                className={`h-5 w-5 ${
                                  branch.headOffice ? "text-white" : "text-[#1A56A0]"
                                }`}
                              />
                            </div>
                            <div>
                              ...
                              <p className="mt-0.5 text-sm text-[#4A5568]">
                                {branch.city}, {branch.region} Region
                              </p>
                ...
                {filteredComingSoon.length > 0 && (
                  <div className="mt-14">
                    ...
                    >
                      {filteredComingSoon.map((branch, i) => (
                        <motion.div
                          key={branch.id}
                          variants={fadeUp}
                          custom={i}
                          ...
                          {branch.expectedOpening && (
                            <div className="mt-4 flex items-center gap-1.5 rounded-lg bg-[#1A56A0]/5 px-3 py-2">
                              ...
                              <span className="text-xs font-semibold text-[#1A56A0]">
                                Expected: {branch.expectedOpening}
                              </span>
                            </div>
                          )}
